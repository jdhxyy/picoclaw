	...
	for ts.currentIteration() < ts.agent.MaxIterations || len(pendingMessages) > 0 || func() bool {
		...
		for i, tc := range normalizedToolCalls {
			...
			if len(toolResult.Media) > 0 {
				switch toolResult.MediaDispatch {
			...
			if toolResult.MediaDispatch == tools.MediaDispatchToLLM && len(toolResult.Media) > 0 {
				// For LLM-bound media, mark content as [image] and include base64 data
				toolResultMsg.Content = "[image]"
				toolResultMsg.Media = toolResult.Media
			} else {
				toolResultMsg.Content = contentForLLM
				if len(toolResult.Media) > 0 && toolResult.MediaDispatch != tools.MediaDispatchToLLM {
					toolResultMsg.Media = toolResult.Media
				}
			}
